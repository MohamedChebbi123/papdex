...
import { updateFile } from "@/components/file_service/file"
...
export function FileBulkMove({ open, onOpenChange, fileIds, fileNames, currentFolderId, subjectId, onMoved }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (fileIds.length === 0 || folderId === "") return
    setLoading(true)
    try {
      const targetId = Number(folderId)
      await Promise.all(fileIds.map(id => updateFile(id, fileNames[id], targetId)))
      onMoved?.(targetId)
      onOpenChange(false)
    } finally {
      setLoading(false)
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-xl border bg-background shadow-xl transition-all duration-150 data-ending-style:opacity-0 data-ending-style:scale-95 data-starting-style:opacity-0 data-starting-style:scale-95">
            <div className="p-6">
              <Dialog.Title className="text-base font-semibold">{t("modal.fileBulkMove.title")}</Dialog.Title>
              <Dialog.Description className="mt-1 text-sm text-muted-foreground">
                {t("modal.fileBulkMove.description", { count: fileIds.length })}
              </Dialog.Description>
